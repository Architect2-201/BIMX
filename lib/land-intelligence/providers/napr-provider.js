            ...
            try {
              ...
              if (searchRes.ok) {
                ...
                if (searchData && searchData.status && searchData.result && searchData.result.length > 0) {
                  // Only accept a result whose name or description matches requested cadastral variants
                  const exactByName = searchData.result.find(r => {
                    const n = (r.name || '').trim();
                    return variants.some(v => v === n);
                  });
                  const exactByText = !exactByName && searchData.result.find(r => {
                    const txt = ((r.resulttext || '') + ' ' + (r.descript || '')).trim();
                    return variants.some(v => txt.includes(v));
                  });
                  const exact = exactByName || exactByText;
                  if (exact) {
                    matchedItem = exact;
                    break;
                  }
