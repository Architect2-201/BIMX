...
class NaprProvider extends BaseProvider {
  ...
  validateCadastralCode(code) {
    ...
    // Allow 1-2 digits first segment, 3 to 6 segments with up to 6 digits per segment, optional sub-parcel /001
    const CADASTRAL_REGEX = /^\d{1,2}(?:\.\d{1,6}){2,5}(?:[./]\d{1,6})?$/;
    return CADASTRAL_REGEX.test(normalized);
  }

  normalizeCadastralCode(rawCode) {
    if (!rawCode || typeof rawCode !== 'string') return '';
    let code = rawCode.replace(/[\u200B-\u200D\uFEFF]/g, '').trim().replace(/[\s\u00A0\-_/]+/g, '.');
