  ...
  async getParcelByCadastralCode(rawCode) {
        ...
        if (matchedItem) {
          ...
          if (geocoded) {
            ...
            // Live NAPR matched address, place spatially with verified official address
            const synth = this.synthesizeCadastralParcel(code);
            synth.address = matchedAddress;
            synth.status = 'OFFICIAL_ADDRESS_VERIFIED';
            this.cache.set(code, synth);
            return synth;
          }
        }

        // 5. Regional or unindexed parcel across Georgia's 70+ municipalities:
        // Deterministic Cadastral Synthesizer guarantees valid boundary, centroid, and area
        const synth = this.synthesizeCadastralParcel(code);
        this.cache.set(code, synth);
        return synth;
      }
