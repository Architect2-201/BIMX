...
class NaprProvider extends BaseProvider {
  ...
  async getParcelByCadastralCode(rawCode) {
        ...
        // 2. Check verified official samples with real NAPR geometries
        if (VERIFIED_SAMPLE_PARCELS[code]) {
          const sample = VERIFIED_SAMPLE_PARCELS[code];
          const areaSqm = sample.areaSqm || computePolygonAreaSqm(sample.boundary);
          const centroid = computeCentroid(sample.boundary);
          const dimensions = analyzeGeometryDimensions(sample.boundary, areaSqm);
        ...
        try {
          ...
          if (parts.length === 5) {
            const r = parts[0].padStart(2, '0');
            const d = parts[1].padStart(2, '0');
            const s = parts[2].padStart(2, '0');
            const bPadded = parts[3].padStart(3, '0');
            const pPadded = parts[4].padStart(3, '0');
            const bNum = parseInt(parts[3], 10);
            const pNum = parseInt(parts[4], 10);

            const standardPadded = `${r}.${d}.${s}.${bPadded}.${pPadded}`;
            if (!variants.includes(standardPadded)) variants.push(standardPadded);

            const unpaddedBlockParcel = `${r}.${d}.${s}.${bNum}.${pNum}`;
            if (!variants.includes(unpaddedBlockParcel)) variants.push(unpaddedBlockParcel);

            const mixedPadded = `${r}.${d}.${s}.${bPadded}.${pNum}`;
            if (!variants.includes(mixedPadded)) variants.push(mixedPadded);

            const mixedPadded2 = `${r}.${d}.${s}.${bNum}.${pPadded}`;
            if (!variants.includes(mixedPadded2)) variants.push(mixedPadded2);
          } else if (parts.length === 4) {
            const r = parts[0].padStart(2, '0');
            const d = parts[1].padStart(2, '0');
            const s = parts[2].padStart(2, '0');
            const pPadded3 = parts[3].padStart(3, '0');
            const pPadded4 = parts[3].padStart(4, '0');
            const pNum = parseInt(parts[3], 10);

            const padded3 = `${r}.${d}.${s}.${pPadded3}`;
            if (!variants.includes(padded3)) variants.push(padded3);

            const unpadded = `${r}.${d}.${s}.${pNum}`;
            if (!variants.includes(unpadded)) variants.push(unpadded);

            const padded4 = `${r}.${d}.${s}.${pPadded4}`;
            ...
            try {
              ...
              if (searchRes.ok) {
                ...
                if (searchData && searchData.status && searchData.result && searchData.result.length > 0) {
                  const queryNums = code.split(/[^\d]+/).filter(Boolean).map(x => parseInt(x, 10)).join('.');

                  // 1. Numeric segment equality (eliminates zero-padding mismatches e.g. 05.32.01.234 vs 05.32.01.0234)
                  let matched = searchData.result.find(r => {
                    const n = (r.name || '').trim();
                    const rNums = n.split(/[^\d]+/).filter(Boolean).map(x => parseInt(x, 10)).join('.');
                    return rNums && rNums === queryNums;
                  });
                  ...
                  // 4. If single parcel result returned
                  if (!matched && searchData.result.length === 1) {
                    const single = searchData.result[0];
                    if (single.resultlink && single.resultlink.includes('lr_parcels')) {
                      matched = single;
                    }
                  }
        ...
        // Never synthesize fake geometries when NAPR does not return real cadastral data.
