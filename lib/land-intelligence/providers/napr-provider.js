...
class NaprProvider extends BaseProvider {
  ...
  normalizeCadastralCode(rawCode) {
    ...
    // Remove prefixes like "ს/კ", "საკადასტრო", "№", "N", "კოდი:"
    ...
    // If unit/apartment code (6+ segments in Tbilisi), take parent 5 segments for parcel geometry
  ...
  async getParcelByCadastralCode(rawCode) {
    ...
    // 3. Attempt live NAPR search with AbortController timeout (6.5s)
    try {
      ...
      if (searchRes.ok) {
        ...
        if (searchData.status && searchData.result && searchData.result.length > 0) {
          ...
          if (geomLink) {
            ...
            const geomTimeoutId = setTimeout(() => geomController.abort(), 6500);
