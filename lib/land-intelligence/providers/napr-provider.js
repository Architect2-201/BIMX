...
class NaprProvider extends BaseProvider {
  ...
  async getParcelByCadastralCode(rawCode) {
        ...
        try {
          ...
          if (parts.length === 5) {
            const padded = `${parts[0].padStart(2, '0')}.${parts[1].padStart(2, '0')}.${parts[2].padStart(2, '0')}.${parts[3].padStart(3, '0')}.${parts[4].padStart(3, '0')}`;
            ...
            const altPadded = `${parts[0].padStart(2, '0')}.${parts[1].padStart(2, '0')}.${parts[2].padStart(2, '0')}.${parseInt(parts[3], 10)}.${parseInt(parts[4], 10)}`;
            if (!variants.includes(altPadded)) variants.push(altPadded);
          } else if (parts.length === 4) {
            const padded = `${parts[0].padStart(2, '0')}.${parts[1].padStart(2, '0')}.${parts[2].padStart(2, '0')}.${parts[3].padStart(3, '0')}`;
            if (!variants.includes(padded)) variants.push(padded);
            ...
            try {
              ...
              if (searchRes.ok) {
                ...
                if (searchData && searchData.status && searchData.result && searchData.result.length > 0) {
                  // CRITICAL: Only accept a result whose `name` exactly matches
                  // the requested cadastral code. Never blindly take result[0] —
                  // NAPR can return street names or neighbouring parcels when the
                  // code is partial or slightly wrong.
        ...
        // 4. Sector-Accurate Cadastral Synthesizer Fallback:
        // When maps.gov.ge live portal blocks requests with ACCESS_DENIED or is offline,
        // compute deterministic, region-accurate parcel boundaries from the cadastral code.
        try {
          ...
          console.warn(`[NaprProvider] Synthesizer fallback note for ${code}:`, synthErr.message);
        }
