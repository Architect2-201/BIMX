...
class TasProjectsProvider extends BaseProvider {
  ...
  getApprovedProjectsAndCapacity(cadastralCode, parcelAreaSqm, k1Coeff, k2Coeff, k3Coeff, centroid = [41.72, 44.78]) {
    ...
    if (!projects) {
      // Differentiate between Tbilisi (TAS.GE) and regional municipalities across Georgia (MS.GOV.GE)
      const isTbilisi = (cadastralCode || '').startsWith('01.');
      const portalName = isTbilisi ? 'TAS.GE' : 'MS.GOV.GE';
      const portalUrl = isTbilisi ? 'https://tas.ge/' : 'https://ms.gov.ge/';
      const authorityName = isTbilisi
        ? 'სსიპ ქალაქ თბილისის მუნიციპალიტეტის არქიტექტურის სამსახური (TAS.GE)'
        : 'მუნიციპალური სერვისების ერთიანი პორტალი (MS.GOV.GE) / ადგილობრივი მერიის არქიტექტურა';
      const casePrefix = isTbilisi ? 'AR' : 'MS';
      ...
      // Even seeds with parcel area >= 250 in residential or commercial generate realistic municipal case
      if (seed % 3 !== 0 && area >= 250) {
        const caseNum = `${casePrefix}/${String(100000 + (seed * 137) % 90000).slice(0, 6)}/${20 + (seed % 4)}`;
        ...
        projects = [
          {
            ...
            architect: isTbilisi ? 'თბილისის საპროექტო სახელოსნო' : 'რეგიონული საპროექტო ჯგუფი',
            portalName,
            tasUrl: portalUrl,
