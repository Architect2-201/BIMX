...
function parseWktPolygon(wkt) {
  ...
  while ((m = coordRegex.exec(clean)) !== null) {
    ...
    let lat, lng;
    // In Georgia, Longitude is between 39.5 and 47.5, Latitude is between 41.0 and 43.6
    if (p1 > 43.6 && p2 < 43.6) {
      // p1 is Longitude, p2 is Latitude
      lng = p1;
      lat = p2;
    } else if (p2 > 43.6 && p1 < 43.6) {
      // p2 is Longitude, p1 is Latitude
      lat = p1;
      lng = p2;
    } else {
      // Standard WKT convention: first is X (lng), second is Y (lat)
      lng = p1;
      lat = p2;
    }
    points.push([lat, lng]); // Leaflet [lat, lng]
  }
  return points;
}
